...
const getConversationById = async (req, res) => {
    try {
        ...
        const isMember = conversation.participants.some(p => p.userId._id.toString() === req.user._id.toString());
        if (!isMember) {
 ...
 */
const deleteOrLeaveConversation = async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({ message: "Chưa đăng nhập" });
            return;
        ...
        if (conversation.type === "direct") {
            ...
            conversation.participants = conversation.participants.filter(p => p.userId.toString() !== req.user._id.toString());
        ...
        try {
            ...
            const notificationPromises = participantIds.map((participantId) => {
                if (participantId === req.user._id.toString())
                    return;
                return createNotification(new mongoose.Types.ObjectId(participantId), "group_invite", "Được thêm vào nhóm mới", `${req.user.display_name} đã thêm bạn vào nhóm "${groupName}"`, conversation._id, req.user._id);
            });
...
const addGroupMembers = async (req, res) => {
    try {
        ...
        if (action === 'remove') {
            ...
            // Remove members
            conversation.participants = conversation.participants.filter(p => !userIds.includes(p.userId.toString()));
            if (conversation.participants.length === 0) {
                await Message.deleteMany({ conversationId: conversation._id });
                await Conversation.findByIdAndDelete(conversationId);
                res.status(200).json({ message: "Nhóm đã bị giải tán" });
        ...
        for (const userId of userIds) {
            ...
            const alreadyInGroup = conversation.participants.some(p => p.userId.toString() === userId);
            if (alreadyInGroup) {
        ...
        for (const userId of userIds) {
            await createNotification(new mongoose.Types.ObjectId(userId), "group_invite", "Được thêm vào nhóm", `${req.user.display_name} đã thêm bạn vào nhóm "${groupName}"`, conversation._id, req.user._id);
        }
        res.status(200).json({
