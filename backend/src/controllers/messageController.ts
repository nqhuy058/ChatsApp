...
const sendMessage = async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    ...
    if (existingConvId && mongoose.Types.ObjectId.isValid(existingConvId)) {
      ...
      if (sender.toString() === recipientId) return res.status(400).json({ message: "Không thể tự gửi tin nhắn cho chính mình" });
    ...
    if (!conversation) return res.status(400).json({ message: "Cần cung cấp conversationId hoặc recipientId hợp lệ" });
    const isMember = conversation.participants.some((p: any) => p.userId.toString() === sender.toString());
    if (!isMember) return res.status(403).json({ message: "Bạn không phải thành viên cuộc trò chuyện này" });
...
const toggleReaction = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const updatedMessage = await Message.findOneAndUpdate(
      { _id: messageId, "reactions": { $elemMatch: { userId: req.user._id, emoji: emoji } } },
      { $pull: { "reactions": { userId: req.user._id, emoji: emoji } } },
      { new: true }
    );
    ...
    const newMessage = await Message.findByIdAndUpdate(
      messageId,
      { $push: { reactions: { userId: req.user._id, emoji: emoji, createdAt: new Date() } } },
      { new: true }
    ).populate("sender", USER_POPULATE_FIELDS_MINIMAL); // SỬA: populate 'sender'
...
const searchMessages = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const results = messages
      .filter(msg => msg.content && removeVietnameseAccents(msg.content).includes(removeVietnameseAccents(req.query.q as string || '')))
      .map(msg => {
        // ...
        return {
          //...
          senderId: (msg.sender as any)._id.toString(), // SỬA: lấy từ msg.sender
          senderName: (msg.sender as any).display_name, // SỬA: lấy từ msg.sender
